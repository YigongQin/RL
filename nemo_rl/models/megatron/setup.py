...
def enable_batch_invariant_mode(config: PolicyConfig) -> None:
    ...
    # Pin (not setdefault): a preset large workspace makes te_native silently
    # non-invariant on the aten GEMM path. Recipe env_vars should already set
    # these via Ray before process start; re-pin here for local/dev entrypoints.
    ...
    already_on = is_batch_invariant_mode_enabled()
    collective = megatron_cfg["batch_invariant_collective"]
    backend = megatron_cfg["batch_invariant_backend"]
    enable_mcore_batch_invariant_mode(backend=backend, collective=collective)
    if not already_on:
        cuda_state = (
            "after CUDA init (handles may retain workspace)"
            if torch.cuda.is_initialized()
            else "before CUDA init"
        )
        print(
            f"[batch_invariant_mode] enabled backend={backend!r} "
            f"collective={collective!r} ({cuda_state}; "
...
def validate_and_set_config(
    ...
    # Backstop if a caller skipped the early worker-init hook. Prefer calling
    # enable_batch_invariant_mode() before torch.cuda.set_device.
